...
v3.1 fix:
  guided_filter_mask     — guide image kept as float32 (NO uint8 quantization)
                           to prevent staircase / aliasing along edges.
"""
import numpy as np
...
def guided_filter_mask(
    ...
    v3.1 anti-aliasing fix:
      Guide is passed as float32 directly — NOT converted to uint8 first.
      The old uint8 quantization caused 256-step banding in the guide image
      which the filter faithfully reproduced as staircase edges in the mask.
    """
    import cv2
    # Keep guide as float32 — critical for sub-pixel gradient accuracy
    ...
    try:
        ...
        # Fallback: bilateral filter (no ximgproc)
...
def trimap_guided_matting(
    ...
    Memory-safe: uses scipy.spatial.cKDTree; falls back to subsampled brute-force.
    """
    ...
    unk_colors= guide[unk_ys, unk_xs].astype(np.float32)
    fg_colors = guide[fg_ys, fg_xs].astype(np.float32)
    bg_colors = guide[bg_ys, bg_xs].astype(np.float32)
...
def _matting_kdtree(
    ...
    for start in range(0, n_unk, BATCH):
        ...
        pos_b = np.stack([unk_ys[start:end], unk_xs[start:end]], axis=1).astype(np.float32)
        ...
        for bi in range(end - start):
            ...
            if len(fg_valid) == 0 or len(bg_valid) == 0:
                result[unk_ys[start+bi], unk_xs[start+bi]] = float(soft_mask[unk_ys[start+bi], unk_xs[start+bi]])
                continue
            F_cands = fg_colors[fg_valid[:n_best]]
            B_cands = bg_colors[bg_valid[:n_best]]
            C_p     = col_b[bi]
...
def _matting_bruteforce(
    ...
    for start in range(0, n_unk, BATCH):
        ...
        pos_b = np.stack([unk_ys[start:end], unk_xs[start:end]], axis=1).astype(np.float32)
        col_b = unk_colors[start:end]
        d_fg  = np.linalg.norm(pos_b[:, None, :] - fg_pos[None, :, :], axis=2)
        d_bg  = np.linalg.norm(pos_b[:, None, :] - bg_pos[None, :, :], axis=2)
        for bi in range(end - start):
...
def hair_bg_island_removal(
    ...
    nlabels, labels, stats, _ = cv2.connectedComponentsWithStats(search_area, connectivity=8)
